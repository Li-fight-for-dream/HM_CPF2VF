...
from src.PCANet_trainer.PCA_trainer.PCA_train import fit_pca
from src.PCANet_trainer.engine import fit
...
def _run_one_fold(base_cfg, full_df, folds, fold_idx, base_save_path):
    ...
    # 每折先做 PCA 训练/评估/保存
    print(f"开始训练第{fold_idx + 1}折的 PCA.......")
    fold_cfg.model.pca_n_components = int(fold_cfg.pca.n_components)
    pca_result = fit_pca(
    ...
    fold_cfg.model.pca_model_path = pca_result["model_path"]
    ...
    model = build_resnet_pca(
        ...
        pca_n_components=int(fold_cfg.model.pca_n_components),
        proj_use_layernorm=bool(fold_cfg.model.proj_use_layernorm),
    ).to(fold_cfg.device)
    model.load_pca_from_joblib(fold_cfg.model.pca_model_path)
