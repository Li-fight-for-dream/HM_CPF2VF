...
# python -m src.AANet_trainer.inference --config configs/recipe_AANet.yaml
# 或者 python -m src.AANet_trainer.inference --config configs/recipe_AANet_stage2.yaml
...
def _resolve_archetype_model_path(cfg, root_dir: str) -> str:
    archetype_dir = Path(root_dir) / "archetype"
...
def _run_inference_and_attach(model, loader: DataLoader, source_df: pd.DataFrame, split_name: str, device) -> pd.DataFrame:
    ...
    n = len(source_df)
    if not (
        len(vf_pred_list) == len(vf_true_list) == len(vf_recon_list) == len(md_pred_list) == len(md_true_list) == n
    ):
...
def main():
    ...
    cfg.model.prototype_model_path = _resolve_archetype_model_path(cfg, path_for_eval)
