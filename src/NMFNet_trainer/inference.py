...
# python -m src.NMFNet_trainer.inference --config configs/recipe_NMFNet.yaml
...
def _resolve_nmf_model_path(cfg, root_dir: str) -> str:
    nmf_dir = Path(root_dir) / "nmf"
...
def main():
    ...
    nmf_model_path = _resolve_nmf_model_path(cfg, path_for_eval)
    ...
    model.load_components_from_joblib(nmf_model_path)
    _load_state_dict_flexible(model, eval_model_path, cfg.device)
    nmf_model = _load_nmf_transformer(nmf_model_path)
