...
def main():
    ...
    # 先做 NMF 训练/评估/保存
    cfg.model.num_components = int(cfg.nmf.n_components)
    nmf_result = fit_nmf(
        save_path=save_path,
        train_idx=train_idx,
        df=full_df,
    ...
    cfg.model.nmf_model_path = nmf_result["model_path"]
    ...
    model.load_components_from_joblib(cfg.model.nmf_model_path)

    losses = build_losses(cfg)
    metrics = build_metrics(cfg)

    optimizer = build_optimizer_for_stage_resnet(model, cfg, stage=1)
