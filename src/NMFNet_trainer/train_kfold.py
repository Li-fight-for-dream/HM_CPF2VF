...
from src.NMFNet_trainer.NMF_trainer.NMF_train import fit_nmf
from src.NMFNet_trainer.engine import fit
...
def _build_fold_loaders(cfg, full_df, train_idx, val_idx):
    ...
    # fit 接口需要三元组；kfold 场景无独立测试集，第三个占位
...
def _run_one_fold(base_cfg, full_df, folds, fold_idx, base_save_path):
    ...
    # 每折先做 NMF 训练/评估/保存
    print(f"开始训练第{fold_idx + 1}折的 NMF.......")
    fold_cfg.model.num_components = int(fold_cfg.nmf.n_components)
    nmf_result = fit_nmf(
    ...
    fold_cfg.model.nmf_model_path = nmf_result["model_path"]
    ...
    model = build_resnet_nmf(
        ...
        num_components=int(fold_cfg.model.num_components),
        proj_use_layernorm=bool(fold_cfg.model.proj_use_layernorm),
        w_activation=str(getattr(fold_cfg.model, "w_activation", "softplus")),
    ).to(fold_cfg.device)
    model.load_components_from_joblib(fold_cfg.model.nmf_model_path)
